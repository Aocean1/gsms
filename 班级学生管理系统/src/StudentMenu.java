...
public class StudentMenu extends JFrame {
    ...
    class JPanelUp extends JPanel {
        ...
        public void init() {
            String sql_message = STR."select * from ssms.student where S_ID = \{input_ID};";
            Connection conn = Main.sendConnection();
            PreparedStatement pstmt;
            ResultSet resultSet;
            String no = "", name = "", sex = "", age = "", d_id = "";
            try {
                ...
                d_id = resultSet.getString(5);
            } catch (SQLException e) {
                e.fillInStackTrace();
            }
            JLabel l_message = new JLabel("个人信息:");
            l_message.setFont(new Font("楷体", Font.BOLD, 20));
            JLabel l_no = new JLabel(String.format("学   号:   %s", no));
            JLabel l_name = new JLabel(String.format("姓   名:   %s", name));
            JLabel l_sex = new JLabel(STR."性   别:   \{sex}");
            JLabel l_age = new JLabel(STR."年   龄:   \{age}");
            JLabel l_d_id = new JLabel(STR."学 院 号:   \{d_id}");
            ...
            panel_left.add(l_d_id);
            this.add(panel_left);
            JLabel l_score = new JLabel("各科成绩:");
            l_score.setFont(new Font("楷体", Font.BOLD, 20));
            JPanel panel_right = new JPanel(new BorderLayout());
            String sql_score = STR."select Cname, grade, teacher.name from ssms.student, ssms.teacher, ssms.course, ssms.score where student.S_ID = \{input_ID} and student.S_ID = score.S_ID and score.Cno = course.CNO and course.T_ID = teacher.T_ID;";
