    ...
    class JPanelRight extends JPanel {//放置右侧 主页面板，个人信息面板等
        ...
        class JPanelHome extends JPanel {//主页面板
            ...
            void init() {
                ...
                button_id.addActionListener(e -> {
                    String sql = "SELECT student.*, course.Cname, score.grade, teacher.name FROM" +
                            ...
                            " and student.S_ID = score.S_ID and score.Cno = course.CNO and course.T_ID = teacher.T_ID;";
                    try {
                        search(sql);
                    } catch (Exception e1) {
                        e1.printStackTrace();
            ...
            class DownPanel extends JPanel {//放置查询结果面板
                ...
                void init(String sql) {
                    ...
                    button_alter.addActionListener(e -> {
                        ...
                        alter.add(text_age);
                        JLabel label_d_id = new JLabel("请输入学院号:");
                        alter.add(label_d_id);
                        JTextField text_d_id = new JTextField();
                        alter.add(text_d_id);
                        JButton bt_save = new JButton("保    存");
                        Connection conn = Main.sendConnection();
                        String sql_alter = "update ssms.student set student.age=?, student.D_ID=? where student.S_ID=?;";
                        bt_save.addActionListener(l -> {
                            PreparedStatement pstmt = null;
                            try {
                                pstmt = conn.prepareStatement(sql_alter);
                                pstmt.setInt(1, Integer.parseInt(text_age.getText()));
                                pstmt.setInt(2, Integer.parseInt(text_d_id.getText()));
                    ...
                    button_alter_score.addActionListener(e -> {
                        String sql_cname = "SELECT course.Cname FROM ssms.course where course.T_ID = " +
                                input_ID + " ;";
                        ...
                        if (flag != null && flag != "") {
                            String sql_write_score = "update ssms.score set score.grade = " + Integer.parseInt(flag)
                                    + " where score.S_Id = " + sign_id + " and score.Cno = (SELECT course.Cno FROM ssms.course where course.T_ID = " + input_ID+");";
                            Connection conn = Main.sendConnection();
                            try {
            ...
            void update_page() {
                ...
                String sql_updata = "SELECT student.*, course.Cname, score.grade, teacher.name FROM" +
                        ...
                        "and and score.Cno = course.CNO and course.T_ID = teacher.T_ID;";
                try {
                    search(sql_updata);
                } catch (Exception e1) {
                    e1.fillInStackTrace();
        ...
        class JPanelInsert extends JPanelHome {
            ...
            void init() {
                ...
                String[] label_text = {"学号", "姓名", "性别", "年龄", "学院号"};
                for (int i = 0; i < 5; i++) {
                ...
                button_confirm.addActionListener(e -> {
                    ...
                    try {
                        ...
                        for (int i = 1; i <= 4; i++) {//在score表添加默认成绩0分
                            String sql_score = "insert into ssms.score values(" + jTextFields[0].getText() + "," + i + ", 0);";
...
//                this.removeAll();
            ...
            class JPanelShow extends JPanel {
                ...
                void show_single(int no, String t_no) {//统计每科成绩
                    ...
                    String sql = "select r.*, row_number() over(order by 'subject') as row_rank\n" +
                            ...
                            "from ssms.score, ssms.student,ssms.course\n" +
                            "where score.S_ID = student.S_ID and score.Cno = course.Cno and course.T_ID = " + t_no +
                            " group by score.S_ID, score.grade\n" +
                    ...
                    String sql_1 = "select count(case when score.grade >= 90 then 1 end) as '优秀',\n" +
                            ...
                            "from ssms.score,ssms.course\n" +
                            "where score.Cno = course.Cno and course.T_ID = " + t_no;
                ...
                void show_all() {//统计总成绩
                    ...
                    String sql = "select r.*, row_number() over(order by '平均分') as row_rank\n" +
                            ...
                            "sum(case when score.Cno = 1 then score.grade else 0 end) as 'java',\n" +
                            "sum(case when score.Cno = 2 then score.grade else 0 end) as '数据库',\n" +
                            "sum(case when score.Cno = 3 then score.grade else 0 end) as '算法',\n" +
                            "sum(case when score.Cno = 4 then score.grade else 0 end) as 'C++',\n" +
        ...
        class PanelMessage extends JPanel {//个人信息面板
            ...
            void init() {
                ...
                String[] label_name = {"教工号:", "姓名:", "性别:", "年龄:", "电话:", "密码:", "学院号:"};
                String sql_message = "SELECT teacher.*, course.Cname FROM ssms.teacher, ssms.course where teacher.T_ID = " + input_ID
